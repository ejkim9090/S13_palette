...
COMMENT ON COLUMN "REVIEW_IMAGE"."RFILEPATH" IS '/resources/review/a.jpg';
...
REFERENCES "REVIEW" (
	"RNO"
) ON DELETE CASCADE; -- 후기가 삭제되면 함께 삭제되도록 CASCADE
...
REFERENCES "MEMBER" (
	"MID"
) ON DELETE CASCADE; -- 회원이나 상품이 삭제되면 함께 삭제되도록 CASCADE
...
REFERENCES "REVIEW" (
	"RNO"
) ON DELETE CASCADE; -- 회원이나 후기가 삭제되면 함께 삭제되도록 CASCADE
